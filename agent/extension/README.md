...
数えた結果は、**この拡張から会社のグループウェアへ直接送る。**

## いちばん大事なところ：EXE は要らない

以前は、数えた結果をパソコンの中のソフト（`EIGHT-Agent-Setup.exe`）へ渡し、
そのソフトが送っていた。つまり拡張を入れただけでは何も動かず、
EXE を配って、SmartScreen をくぐって、入れてもらう必要があった。
入れ直し・更新のたびに同じことをやる。そこが重かった。

いまは、社員がすることは2つだけ。

1. グループウェア（mf.8grp.co.jp）にログインする
2. この拡張を入れる

つないだかどうかは、拡張の「オプション」ページで見られる。
...
localStorage も Cookie も見えない。**読めてしまうほうが困る。**
    ...
    ログイン済みの画面  --(1回きりの合言葉)-->  拡張
    拡張  --(合言葉)-->  /api/devices/browser  --(端末専用の資格情報)-->  拡張
    拡張  --(WEB利用)-->  /api/devices/ingest

合言葉は5分で切れ、1回使うと死ぬ。
拡張が持つのは端末専用の資格情報だけで、できることは「自分のぶんを送る」しかない。
社員としては何もできないし、EXE 用の口（更新の確認）も叩けない。

画面から拡張へ話しかけられるのは `externally_connectable` に書いた
`https://mf.8grp.co.jp/*` だけ。ほかのサイトからは届かない。
...
| 持っていない権限 | だから、できないこと |
|---|---|
| `host_permissions`（会社のグループウェアだけ） | ほかのどのページにもコードを差し込めない・通信もできない |
| `nativeMessaging`（無し） | パソコンの中のソフトとつながらない（EXE が要らない） |
...
持っているのは `tabs` `idle` `alarms` `storage` の4つだけ。
`host_permissions` は `https://mf.8grp.co.jp/*` の1つだけで、
**ほかのサイトには1つも通信できない**（ページの中にも入れない）。
`storage` は端末専用の資格情報を置くために要る。
