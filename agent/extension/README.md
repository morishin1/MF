...
ID を入れずに組むと、インストーラはブラウザ連携を設定できずに止まる
（黙って半分だけ入った状態を作らないため）。
...
signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 ^
  /f 会社の証明書.pfx /p パスワード dist\EIGHT-Agent-Setup.exe
```

**署名しないと SmartScreen が止める。**
社員に「警告を無視して実行してください」と案内するのは、
社内のセキュリティ教育として逆効果なので、署名前に配らないこと。
