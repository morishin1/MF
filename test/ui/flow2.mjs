...
console.log("— ① 期日から逆算した最優先（ホーム）—");
{
  ...
  await p.screenshot({ path: "f2-home.png", fullPage: true });
  await p.close();
}
...
console.log("— ② AI提案の選別（ホーム）—");
{
  ...
  await p.screenshot({ path: "f2-prop.png", fullPage: true });
  await p.close();
}
...
console.log("— 見送っても消えない（やること）—");
{
  ...
  check((await p.locator("#req-list").textContent()).includes("テスト送信②です"),
    "自分が頼んだことに「補足」が出る");
  await p.screenshot({ path: "f2-tasks.png", fullPage: true });
  await p.close();
}
...
console.log("— 日報の直し —");
{
  ...
  await p.screenshot({ path: "f2-nippo.png", fullPage: true });
  await p.close();
}
