...
export const NEXT_ACTION_LABEL = {
  ...
  offer_send_pending: "本人へ送付してください", offer_response_pending: "本人の回答を待っています",
 ...
 */
export function nextActionOf(a, nextInterview = null) {
  if (a.status === "todo" || a.status === "scheduling") {
    return { label: "面談を予定してください", cta: "面談を予定する", action: "schedule", kind: "casual" };
  }
  ...
  // 送付そのもの（本人専用URL発行・メール送信）はStage 6で行う（ここではまだcta/actionを出さない）
  ...
  if (a.status === "offer_send_pending") {
    return { label: NEXT_ACTION_LABEL.offer_send_pending, cta: null, action: null };
  }
 ...
 * @param {object} [nextInterview] いちばん直近の、実施前の面談（{scheduledAt, kind}）
 */
export const shapeApplicant = (a, nextInterview = null) => {
  const next = nextActionOf(a, nextInterview);
  return {
