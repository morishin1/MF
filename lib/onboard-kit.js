...
import { flowItems, flowOf } from "./hr-flow.js";
import {
  DOCS, COMPANY_DOCS, asChecklistItems, companyChecklistItems, docByTitle, ownerOf,
} from "./onboard-docs.js";
...
// 契約書（雇用・業務委託）はここから外した。
// lib/onboard-docs.js の COMPANY_DOCS が鍵付きで受け持つ。
// 鍵が無いままだと、管理画面から契約書のファイルを付けられない
// （どの書類か分からないので、Drive のどこに置くかも決まらない）
...
const LEGACY_KEYS = new Map([
  ...
  // 会社側の準備は lib/hr-flow.js の鍵に寄せる。
  // 昔の鍵（prep_pc など）へ結び直すと、066 で寄せたばかりのものが
  // また古い形に戻って、入退社の画面に同じ作業が2行並ぶ
  ["PC・備品の貸与", "on_it_pc"],
  ["PCの準備・初期設定", "on_it_pc"],
  ["Slack アカウント発行", "on_it_slack"],
  ["メールアカウントの発行", "on_it_mail"],
  ["グループウェアの招待", "on_it_slack"],
  ["勤怠（タイムカード）の登録", "on_it_perm"],
  ["初日の受け入れ準備", "on_mgr_orient"],
  ["初日の受け入れ・顔合わせ", "on_mgr_orient"],
]);

// 本人フォームが受け持つ項目だけ。
// 会社側の準備（PREP_TASKS）はここに入れない。入れると、上の LEGACY_KEYS で
// いまの手順（lib/hr-flow.js）へ寄せたものが、また古い鍵に引き戻される
const KEYED_BY_TITLE = new Map(FORM_ITEMS.map((i) => [i.title, i.item_key]));

/**
 * その人の手続きを1つ引く。
 *
 * ■ なぜ maybeSingle を使わないのか
 *
 *   ここは長いあいだ
 *
 *     .eq("employee_id", id).eq("kind", "onboarding").maybeSingle()
 *
 *   と書いてあり、コメントには「1人1つ（employee_id, kind で一意）」と
 *   書いてあった。ところが、その一意制約はどの SQL にも無かった。
 *
 *   行が2つできると maybeSingle は **エラーを返して data を null にする**。
 *   呼ぶ側はどこも error を見ていなかったので、
 *
 *     ・本人の画面 … 手続きが無いことになり、6つの書類すべてに
 *       「この書類は、いまのチェックリストに結び付いていません」と出て、
 *       出す口（アップロード）が1つも出なくなる
 *     ・作る側     … 「まだ無い」と判断して、もう1つ作る
 *
 *   となる。2つになった時点で本人は何も出せなくなり、
 *   管理者が操作するたびに3つ4つと増えていく。
 *
 *   増えてしまったぶんは db/065 で片付ける（一意制約もそこで付ける）。
 *   このコードは、片付ける前の環境でも動くようにしてある。
 *
 * @returns {Promise<{row:object|null, dup:number, error:object|null}>}
 */
export async function findProcedure(sb, employeeId, kind = "onboarding", cols = "*") {
  const { data, error } = await sb.from("gw_procedures").select(cols)
    .eq("employee_id", employeeId).eq("kind", kind)
    .order("created_at", { ascending: true })
    .limit(10);
  if (error) return { row: null, dup: 0, error };
  const rows = data || [];
  if (rows.length <= 1) return { row: rows[0] || null, dup: 0, error: null };

  // 2つ以上ある。いちばん古いものを正とする。
  // あとから増えたほうは空であることが多く、書類やファイルは
  // 最初に作られたほうに付いている
  console.error(`[onboard] ${kind} の手続きが ${rows.length} 件あります`
    + `（employee_id=${employeeId}）。db/065_onboarding_one.sql を流してください`);
  return { row: rows[0], dup: rows.length, error: null };
}
...
export async function createOnboardingKit(sb, ctx, user, emp, master) {
  ...
  try {
    // 1) 手続き。1人1つ。すでにあれば作り直さない。
    //
    //    ここで「無い」と読み違えると、2つ目を作ってしまう。
    //    2つになると本人の画面から出す口が消えるので、
    //    読めなかったときは作らずに降りる（findProcedure を見ること）
    const found = await findProcedure(sb, emp.id, "onboarding", "id");
    if (found.error) throw new Error(found.error.message);

    let procedureId = found.row?.id || null;
    if (!procedureId) {
    ...
    const rows = [
      ...
      // 会社が渡す書類（契約書）。本人が出す書類のすぐ下に並べる
      ...companyChecklistItems(emp.employment_type).map((i, n) => ({ ...i, sort_order: 200 + (n + 1) * 10 })),
      ...base.map((i, n) => ({ ...i, sort_order: 300 + (n + 1) * 10 })),
      // 会社側の準備は lib/hr-flow.js が正。
      // ここで別の一覧（PREP_TASKS）を作ると、入退社の画面に
      // 「PCの準備・初期設定」と「会社PCの準備」が2行並ぶ
      ...flowItems("onboarding").map((f) => ({
        item_key: f.key, title: f.title, category: "task",
        owner: f.role, phase: f.phase,
        required: true, share_with_advisor: false,
        sort_order: 500 + f.sortOrder,
      })),
    ];
    ...
    await ensureDocItems(sb, ctx.tenantId, procedureId, emp.employment_type);
 ...
 */
export async function ensureDocItems(sb, tenantId, procedureId, employmentType = null) {
  const out = { linked: 0, removed: 0, added: 0, error: null };
  if (!procedureId) {
    out.error = "手続きがありません";
    return out;
  }

  // 読めなかったことを、黙って「0件」にしない。
  //
  // ここを `data` だけ取って捨てていたので、読めていないのか
  // 本当に空なのかが区別できず、本人の画面には
  // 「この書類は、いまのチェックリストに結び付いていません」としか
  // 出なかった。何を直せばよいのか、どこにも書かれていない状態になる
  const { data: items, error: readErr } = await sb.from("gw_procedure_items")
    .select("id, item_key, title, status, sort_order")
    .eq("procedure_id", procedureId).limit(300);
  if (readErr) {
    out.error = `チェックリストを読めません: ${readErr.message}`;
    return out;
  }
  if (!items) {
    out.error = "チェックリストを読めません（返事が空でした）";
    return out;
  }
  ...
  for (const it of orphan) {
    ...
    if (!taken.has(key)) {
      ...
      if (doc) {
        ...
        // 契約書は会社が渡すもの。本人の担当に付け替えない
        patch.owner = ownerOf(doc);
        patch.share_with_advisor = !!doc.advisor;
      }
      // 会社側の準備なら、いまの手順の名前・担当・段階に合わせる。
      // 鍵だけ付け替えて中身を昔のままにすると、
      // 「PCの準備・初期設定（担当：人事）」のように、
      // 名前と担当が食い違ったまま画面に並ぶ
      const f = flowOf(key);
      if (f) {
        patch.title = f.title;
  ...
  // 3) 定義にあるのに無い書類を足す。
  //    契約書は、雇用と業務委託でどちらか一方だけ。
  //    片方がもう入っているなら、もう片方は足さない
  //    （上の結び直しで、昔の「業務委託契約書の締結」も鍵付きになっている）
  const hasGyomu = taken.has("doc_contract_gyomu");
  const hasKoyo = taken.has("doc_contract");
  const company = (hasGyomu || hasKoyo)
    ? []
    : companyChecklistItems(employmentType);

  const missing = [...asChecklistItems(), ...company].filter((r) => !taken.has(r.item_key));
  if (missing.length) {
    const order = (key) => {
      const n = DOCS.findIndex((d) => d.key === key);
      if (n >= 0) return 100 + (n + 1) * 10;
      return 200 + (COMPANY_DOCS.findIndex((d) => d.key === key) + 1) * 10;
    };
    const { error } = await sb.from("gw_procedure_items").insert(
      missing.map((r) => ({
        tenant_id: tenantId,
        ...
        required: r.required !== false,
        share_with_advisor: !!r.share_with_advisor,
        sort_order: order(r.item_key),
      })));
    // 足せなかったら、そう言う。
    //
    // ここで黙って降りると、本人の画面には出す口が1つも出ないのに、
    // 原因はサーバのログにも残らない。列が足りない・権限が無い、
    // どちらも「足りない書類が永久に出てこない」という同じ見え方になる
    if (error) out.error = `書類の項目を足せません: ${error.message}`;
    else out.added = missing.length;
  }
