...
//
// ■ 本人にフォルダを選ばせない
//   本人は、mf の画面で書類ごとのボタンを押して、開いた Google ドライブの
//   フォルダに入れるだけ。どのフォルダが開くかは、この定義が決める。
//   フォルダを選ばせると「社労士に渡す 04 に入っていない」が起きる。
//   （ドライブを渡せない相手には、mf からのアップロードも残してある。
//     そちらを通ったものは、下の driveFileName で名前を付け直す）
//
// ■ フォルダ構成（1人1つ）
//   01_採用・履歴書
//   02_労働条件・契約
...
export const DOCS = [
  ...
  },
  {
    key: "doc_mynumber",
    title: "マイナンバー確認書類",
    // ここでは受け取らない。番号は社労士側で直接収集・管理する（lib/mynumber.js）。
    // 項目は残す（本人に「社労士から案内が来る」と伝えるため）が、
    // アップロード口は閉じ、出したかどうかで手続きの段階を止めない
    desc: "このシステムでは受け取りません。社労士から直接、提出方法のご案内があります。",
    folder: "03",
    sensitive: true,
    collect: false,
    template: null,
 ...
 *
 * ■ 出す向きが逆
 *   DOCS は本人 → 会社。こちらは会社 → 本人。
 *   同じチェックリスト（gw_procedure_items）に入れるが、owner は hr。
 *   本人はあげられない（api/onboarding/upload.js が owner を見て弾く）。
 *
 * ■ 本人だけが見られる
 *   置き場所は 02_労働条件・契約。ここは本人にも共有しない
 *   （lib/hr-drive.js の EMPLOYEE_FOLDER_KEYS に 02 は入っていない）。
 *   代わりに、mf のマイページから本人だけが開ける。
 *   gw_procedure_files の RLS が「自分の手続きのファイル」を通すので、
 *   本人・人事・管理者以外には渡らない。
 *
 * ■ 社労士には既定で見せない
 *   資格取得届に契約書そのものは要らない。
 *   必要な案件だけ、管理画面の「社労士」の丸を人が付ける。
 */
export const COMPANY_DOCS = [
  {
    key: "doc_contract",
    title: "雇用契約書・労働条件通知書",
    aliases: ["雇用契約書の締結", "雇用契約書", "労働条件通知書"],
    desc: "会社が用意します。署名・押印したものをここに入れます。",
    folder: "02",
    owner: "hr",
    template: null,
    required: true,
  ...
  {
    key: "doc_contract_gyomu",
    title: "業務委託契約書",
    aliases: ["業務委託契約書の締結"],
    desc: "業務委託の方はこちら。会社が用意します。",
    folder: "02",
    owner: "hr",
    template: null,
    required: true,
...
/** 本人が出すものと、会社が渡すもの。鍵は全体で一意 */
const ALL_DOCS = [...DOCS, ...COMPANY_DOCS];

export const docOf = (key) => ALL_DOCS.find((d) => d.key === key) || null;

/** その書類は誰の持ちものか。既定は本人 */
export const ownerOf = (doc) => doc?.owner || "employee";
...
export const docByTitle = (title) => {
  ...
  if (!t) return null;
  return ALL_DOCS.find((d) => d.title === t || (d.aliases || []).includes(t)) || null;
};
...
/** 本人が出す書類 */
export const asChecklistItems = () => DOCS.map(asItem);

/**
 * 会社が渡す書類。
 *
 * 業務委託契約書は、雇用の人には作らない（逆も同じ）。
 * 両方並べると、どちらかに必ず「対象外」を付けることになる
 */
export const companyChecklistItems = (employmentType) => {
  const key = employmentType === "業務委託" ? "doc_contract_gyomu" : "doc_contract";
  return COMPANY_DOCS.filter((d) => d.key === key).map(asItem);
};
