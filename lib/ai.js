...
function nippoToText(n) {
  ...
  const parts = [
    ["日付", n.work_date],
    ["朝に決めた最優先", n.top_priority],
    ["朝の時点で困っていたこと", n.morning_note],
    ["① 今日のKPI", kgi],
    ["② やること・できたこと・できなかった理由",
      rows(n.work_items, [["task", "やること"], ["result", "できたこと"],
                          ["undone_reason", "できなかった理由"]])],
    ["③ 相談事項", n.consult_note],
    ["④ 明日やること", n.tomorrow_plan],
    ["調子（旧）", n.mood],
    ["困ったこと・報告相談（旧）", n.no_issues ? "特になし"
      : rows(n.issues, [["issue", "問題"], ["action_taken", "自分でやったこと"],
                        ["consulted", "相談相手"], ["next_action", "次の行動"]])],
    ["改善・学び（旧）", [(n.improve_tags || []).join("・"), n.challenge].filter(Boolean).join(" / ")],
    ["顧客・チームのためにしたこと（旧）", n.contribution],
