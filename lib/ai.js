...
function nippoToText(n) {
  ...
  const parts = [
    ["日付", n.work_date],
    ["調子", n.mood],
    ["今日の目的", [n.purpose_area, n.purpose].filter(Boolean).join("・")],
    ["今週のKGI", n.kgi_week],
    ["週の進捗", n.progress_total ? `${n.progress_done ?? 0} / ${n.progress_total}` : ""],
    ["今日のKPI", rows(n.kpis, [["name", "指標"], ["goal", "目標"], ["actual", "実績"]])],
    ["チームKGI", rows(n.team_kgi, [["name", "指標"], ["goal", "目標"], ["actual", "実績"]])],
    ["今日進んだこと", n.today_work],
    ["次工程へ進めた結果", n.funnel],
    ["引き継ぎ", rows(n.handoff, [["to_name", "誰に"], ["what", "何を"], ["state", "状態"]])],
    ["困っていること", n.struggle],
    ["止まっていること", rows(n.stuck, [["item", "案件"], ["pos", "工程"], ["reason", "理由"], ["ball", "ボール"], ["next", "次の一手"]])],
    ["未達の原因", n.miss_reason],
    ["挑戦・改善", n.challenge],
    ["小さな成功", n.small_win],
    ["貢献", n.contribution],
    ["明日やること", n.tomorrow_plan],
  ];
