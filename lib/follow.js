 ...
 * 残業時間・日報の文字数・AIとの会話量は数えない。
 */
export function rankings({ staff, nippos, kpis, prevNippos = [], evals = [] }) {
  const per = new Map();
  for (const e of staff) {
    per.set(e.user_id, {
      userId: e.user_id, name: e.display_name,
      results: 0, actions: 0, improves: 0, customer: 0, customerOf: 0, days: 0,
    });
  }
  ...
  for (const n of nippos) {
    ...
    u.results += items.filter(isDone).length;
    // 行動 … やったことの件数
    u.actions += items.length;
    // 改善 … 未完了を理由まで書いて、明日につないだ日。
    //   日報を絞ったときに「改善タグ」の欄を外したので、
    //   チェックの有無ではなく、実際に書かれた内容から数える
    if (items.some((w) => w.undone_reason) && n.tomorrow_plan) u.improves++;
  }

  // 顧客価値 … 「顧客・チームのためにしたこと」の欄を外したので、
  // 文章を読んでいるAIが付けた点から数える。6点以上をその日できていたとする
  for (const e of evals) {
    const u = per.get(e.user_id);
    const s = e?.scores?.customer_focus;
    if (!u || !s || s.status !== "evaluated" || !Number.isFinite(s.score)) continue;
    u.customerOf++;
    if (s.score >= 6) u.customer++;
  }
  ...
  for (const n of prevNippos) {
    ...
    prevResults.set(n.user_id, c + (n.work_items || []).filter(isDone).length);
  }
