...
function weeklyPrompt({ metrics, nippos, evals, review }) {
  ...
  const days = nippos.map((n) => {
    ...
    return [
      ...
      n.top_priority ? `  最優先：${n.top_priority}` : "",
      items.length
        ? `  成果：${items.filter(isDone).map((w) => `${w.task}${w.result ? `→${w.result}` : ""}`).join(" / ") || "（結果の記載なし）"}`
        : "  成果：（記載なし）",
      items.some((w) => w.undone_reason)
        ? `  未完了：${items.filter((w) => w.undone_reason).map((w) => `${w.task}（${w.undone_reason}）`).join(" / ")}`
        : "",
      n.consult_note ? `  相談：${n.consult_note}` : "",
      // 旧い形式の日報も読めるようにしておく
      !n.no_issues && (n.issues || []).length
        ? `  困りごと（旧）：${(n.issues || []).map((i) =>
            `${i.issue}（自分で:${i.action_taken || "—"} / 相談:${i.consulted || "なし"} / 次:${i.next_action || "—"}）`).join(" / ")}`
        : "",
      n.contribution ? `  顧客・チーム（旧）：${n.contribution}` : "",
