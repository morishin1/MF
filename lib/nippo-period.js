...
export const WEEKLY_PROMPT_VERSION = "weekly_eval_v1";
export const MONTHLY_PROMPT_VERSION = "monthly_summary_v1";
...
export function weeklyMetrics({ weekStart, nippos, evals }) {
  ...
  return {
    ...
    dailyAvg: perKey,
    dailyAvgTotal: (() => {
      const vals = Object.values(perKey).filter((v) => v.avg !== null).map((v) => v.avg);
      return vals.length ? Math.round((vals.reduce((a, b) => a + b, 0) / (vals.length * 10)) * 100) : null;
    })(),
  };
}
...
/** 材料不足の項目は分母から外して100点満点に按分する */
function withTotal(result) {
  const rated = KEYS
    .map((k) => result.scores?.[k])
    .filter((s) => s && s.status === "evaluated" && Number.isFinite(s.score));
  return {
    ...result,
    total: rated.length
...
export async function summarizeMonth({ metrics, weeks }) {
  const m = metrics;
