 ...
 * 既定に gw.8grp.co.jp を入れているのは、社員のアドレスがこちらのため
 * （s_morita@gw.8grp.co.jp）。8grp.co.jp も一応残す
 */
const shareDomains = () =>
  String(process.env.GDRIVE_SHARE_DOMAINS || "gw.8grp.co.jp,8grp.co.jp")
...
/**
 * 貼り付けられた Drive の URL からフォルダIDを取り出す。
 *
 * ■ なぜ URL を受けるのか
 *   サービスアカウントでフォルダを作らせるには、鍵の設定と、
 *   親フォルダの共有と、権限の確認が要る。
 *   管理者が自分でフォルダを作って共有し、URLを貼るほうが早くて確実。
 *   誰に見せるかも、ドライブの共有画面でそのまま決められる。
 *
 * ■ 受ける形
 *   https://drive.google.com/drive/folders/xxxx?usp=drive_link
 *   https://drive.google.com/open?id=xxxx
 *   xxxx（IDだけ貼られた場合）
 *   ?usp= 以降は捨てる。付いたまま保存すると、リンクが人によって変わる
 */
