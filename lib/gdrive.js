...
// サービスアカウントのメールアドレス。
// フォルダを共有する相手として管理画面に出す。秘密ではない（鍵は別）
export function serviceAccountEmail() {
  try { return creds()?.email || null; } catch { return null; }
}

// サービスアカウントの鍵があるか。保存先IDの有無は問わない
// （カレンダー連携など、Drive のフォルダIDを使わない用途から見るため）
export function hasCredentials() {
  try { return Boolean(creds()); } catch { return false; }
}

export const DRIVE_SCOPE = "https://www.googleapis.com/auth/drive";

// ---- アクセストークン（インスタンス内でキャッシュ） ----
// スコープごとに別のトークンなので、スコープをキーにして持つ
const tokenCache = new Map(); // scope -> { token, exp }

export async function getAccessToken(scope = DRIVE_SCOPE) {
  ...
  tokenCache.set(scope, { token: data.access_token, exp: now + (Number(data.expires_in) || 3600) });
 ...
 * フォルダの状態を見る。設定の切り分け用。
 *
 * サービスアカウントに共有されていないフォルダIDを設定しても、
 * 画面には「フォルダを作れませんでした」としか出ない。
 * 何が足りないのかを、こちらから見に行って言えるようにする。
 *
 * @returns {Promise<{ok:boolean, id, name?, canAddChildren?, owner?, sharedWith?:object[], error?:string}>}
 */
export async function folderCheck(id) {
  if (!id) return { ok: false, id, error: "フォルダIDが設定されていません" };
  try {
    const f = await driveFetch(
      `${API}/files/${encodeURIComponent(id)}?supportsAllDrives=true`
      + `&fields=id,name,mimeType,capabilities(canAddChildren),owners(emailAddress)`);
    if (f.mimeType !== FOLDER_MIME) {
      return { ok: false, id, name: f.name, error: "フォルダではありません" };
    }
    // 共有相手。ドメイン全体に開いていないかを見るため
    let sharedWith = null;
    try {
      const p = await driveFetch(
        `${API}/files/${encodeURIComponent(id)}/permissions?supportsAllDrives=true`
        + `&fields=permissions(type,role,emailAddress,domain)`);
      sharedWith = p.permissions || null;
    } catch { /* 権限一覧が読めなくても、書けるかどうかは分かる */ }

    return {
      ok: !!f.capabilities?.canAddChildren,
      id, name: f.name,
      canAddChildren: !!f.capabilities?.canAddChildren,
      owner: f.owners?.[0]?.emailAddress || null,
      sharedWith,
      error: f.capabilities?.canAddChildren ? null : "このフォルダに書き込めません（編集者で共有してください）",
    };
  } catch (e) {
    const m = String(e.message || e);
    return {
      ok: false, id,
      error: /404|not found|File not found/i.test(m)
        ? "このフォルダが見つかりません。サービスアカウントに共有されていないか、IDが違います"
        : m.slice(0, 200),
    };
  }
}
...
// ---- 共有（フォルダやファイルに、人を追加する） ----
// 社労士に 04_社会保険・労務 だけを見せるのに使う。
// role は reader（閲覧）を既定にする。書き込みまで渡す場面は、いまは無い。
// 同じ人に2回渡しても壊れない（Drive 側が既存の権限を更新する）。
export async function shareWith(fileId, email, role = "reader") {
  if (!fileId || !email) return { skipped: "missing" };
  const data = await driveFetch(
    `${API}/files/${encodeURIComponent(fileId)}/permissions?supportsAllDrives=true&sendNotificationEmail=false&fields=id,emailAddress,role`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return { permissionId: data.id, email: data.emailAddress, role: data.role };
}
