...
export const CRITERIA = {
  ...
  2: [
    ...
    { key: "consultRate",  label: "詰まった日に声を上げている",       need: 80, unit: "%" },
  ...
  3: [
    ...
    { key: "issueWithPlan", label: "未完了を明日につないでいる",      need: 70, unit: "%" },
  ],
  // L3 → L4 … 人と仕組みを動かせる
  4: [
 ...
 * @param {Array} p.evals        その期間の日報AI評価（gw_nippo_ai_evals の completed）
 *
 * ■ 「改善」と「顧客・チーム」はAI評価から取る
 *   以前は日報のチェックボックス（改善タグ）と自由記述欄から数えていた。
 *   日報を朝4つ・夜5つに絞ったときにその欄を外したので、
 *   文章を読まないと分からないものは、文章を読んでいるAIの点から取る。
 *   チェックを付ければ上がる形より、書いた内容で決まるほうが実態に近い。
 */
export function computeMetrics({ workdays, nippos, kpis, items, blockers, evals = [] }) {
  ...
  // 詰まった日に、声を上げられたか。
  //
  // 「困りごとのうち相談した割合」だと、困りごとを書かない人ほど分母が
  // 小さくなり、黙っているほど数字が良く見える。
  // 分母を提出日数にして、「詰まったのに黙っていた日」だけが下がるようにする。
  // 詰まらなかった日は、声を上げる必要がないので満たしたものとして数える。
  const stuck = (n) => (n.work_items || []).some((w) => w.undone_reason);
  const spoke = (n) => Boolean(n.consult_note || n.morning_note
    // 旧形式の日報（036より前）は、困りごとの行から見る
    || (!n.no_issues && (n.issues || []).some((i) => i.issue)));
  const okConsult = nippos.filter((n) => !stuck(n) || spoke(n));
  // 未完了があった日に、明日やることまで書けているか
  const okCarry = nippos.filter((n) => !stuck(n) || Boolean(n.tomorrow_plan));

  const results = nippos.reduce((a, n) => a + (n.work_items || []).filter(isDone).length, 0);
  const works = nippos.reduce((a, n) => a + (n.work_items || []).length, 0);

  // AI評価から取る2つ。点が付いた日だけを分母にする（材料不足の日は数えない）
  const rated = (key) => {
    const vals = evals
      .map((e) => e?.scores?.[key])
      .filter((s) => s && s.status === "evaluated" && Number.isFinite(s.score));
    // 10点満点の6点以上を「その日はできていた」とする。
    // 週次・月次の合格ラインと同じ考え方
    return vals.length ? pct(vals.filter((s) => s.score >= 6).length, vals.length) : null;
  };
  ...
  return {
    ...
    issueCount: nippos.filter(stuck).length,
    consultRate: pct(okConsult.length, nippos.length),
    issueWithPlan: pct(okCarry.length, nippos.length),

    improveRate: rated("feedback_improvement"),
    resultRate: pct(results, works),
    customerRate: rated("customer_focus"),
