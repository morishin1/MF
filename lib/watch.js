...
export const LIMITS = {
  ...
  silentHours: 24,
  // 本人がグループウェアを使っているのに、拡張から届かない時間。
  // 拡張は1分ごとに送るので、これだけ空いたら「外れている」と見てよい
  extSilentMin: 60,
};
...
export function extState(devices, now = Date.now()) {
  ...
  if (mine.some((d) => d.registeredAt)) {
    return { key: "removed", mark: "×", label: "拡張が外れています" };
  }
 ...
 *
 * 本人がその画面を使っているのに、拡張からだけ届かない状態が続いているとき
 */
export function extGone(d, now = Date.now()) {
  ...
  const ext = d.extSeenAt ? Date.parse(d.extSeenAt) : 0;
  if (!beat) return false;                        // そのブラウザ自体を使っていない
  // グループウェアは最近使っている
  if (now - beat > LIMITS.extSilentMin * 60000) return false;
  // なのに拡張からは、しばらく届いていない
  return (now - ext) > LIMITS.extSilentMin * 60000;
}
...
export function issuesOf(p) {
  ...
  if (ext.key === "removed") {
    out.push({
      ...
      what: "登録済みのパソコンで、ブラウザ拡張が外れています",
      next: "本人に確認してください。登録の解除は管理者しかできません",
    });
  }
