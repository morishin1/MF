...
// calendar.events は「予定の読み書き」。カレンダーそのものを作ったり消したり、
// 共有設定を変えたりする権限（calendar）は求めない。
// 社内で入れた予定を本人のカレンダーへ書き出すのに必要な、いちばん狭い範囲。
export const WRITE_SCOPE = "https://www.googleapis.com/auth/calendar.events";

const SCOPES = ["openid", "email", WRITE_SCOPE].join(" ");

/** 保存してある scope 文字列に、書き込みの権限が含まれているか */
export const canWrite = (scope) => String(scope || "").includes(WRITE_SCOPE);
...
/**
 * 社内の予定を Google の形にする。
 * 終日は date（終わりは翌日＝終端を含まない）、時刻つきは dateTime。
 * ここを取り違えると、終日の予定が1日ずれる。
 */
function eventBody(ev) {
  const day = (iso) => new Date(new Date(iso).getTime() + 9 * 3600000).toISOString().slice(0, 10);

  const time = ev.allDay
    ? {
        start: { date: day(ev.startsAt) },
        // 終日は終端を含まないので、表示上の最終日の翌日を渡す
        end: { date: day(new Date(new Date(ev.endsAt).getTime() + 86400000).toISOString()) },
      }
    : {
        start: { dateTime: new Date(ev.startsAt).toISOString(), timeZone: "Asia/Tokyo" },
        end: { dateTime: new Date(ev.endsAt).toISOString(), timeZone: "Asia/Tokyo" },
      };

  return {
    summary: ev.title,
    description: [ev.body, "（社内グループウェアから登録）"].filter(Boolean).join("\n\n"),
    location: ev.location || undefined,
    // 予定の区分が「private」のときは、Google 側でも非公開扱いにする
    visibility: ev.category === "private" ? "private" : "default",
    ...time,
  };
}

/** 新しく入れる。返り値は Google 側の予定 id */
export async function insertEvent(accessToken, ev) {
  const data = await calendarWrite(accessToken, "/calendars/primary/events", "POST", eventBody(ev));
  return { id: data.id, link: data.htmlLink || null };
}

/** 既に入れてあるものを上書きする。向こうで消されていたら null を返す */
export async function updateEvent(accessToken, gcalEventId, ev) {
  try {
    const data = await calendarWrite(
      accessToken, `/calendars/primary/events/${encodeURIComponent(gcalEventId)}`, "PATCH", eventBody(ev),
    );
    return { id: data.id, link: data.htmlLink || null };
  } catch (e) {
    // 向こうで消されている場合は入れ直す。「反映されない」を残さないため
    if (/HTTP 404|HTTP 410/.test(String(e.message))) return null;
    throw e;
  }
}

/** 書き出したものを取り消す。既に無ければ成功扱い */
export async function deleteEvent(accessToken, gcalEventId) {
  const r = await fetch(`${API}/calendars/primary/events/${encodeURIComponent(gcalEventId)}`, {
    method: "DELETE",
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (r.ok || r.status === 404 || r.status === 410) return true;
  const data = await r.json().catch(() => ({}));
  throw new Error(`google_delete_failed: ${data.error?.message || `HTTP ${r.status}`}`);
}

async function calendarWrite(accessToken, path, method, body) {
  ...
  const data = await r.json().catch(() => ({}));
  if (!r.ok) {
    throw new Error(`google_write_failed: ${data.error?.message || ""} HTTP ${r.status}`);
  }
  return data;
}
