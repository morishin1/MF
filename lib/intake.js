...
/**
 * 手当。毎回ちがう言い方で書かれると、あとで数えられない。
 * よくあるものは選べるようにして、当てはまらないものだけ自由記述にする。
 *
 * 保存先は gw_contracts.wage_note（テキスト1列）のまま。
 * 選んだものと自由記述を「、」でつないで入れる。
 * 手当ごとに列を作らないのは、金額の内訳まで持つと
 * 給与計算の仕組みになってしまい、この道具の役目を超えるため。
 */
