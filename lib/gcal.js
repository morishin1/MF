...
export async function createEvent(calendarId, ev) {
  return postEvent(calendarId, eventBody(ev));
}

// 組み立て済みの本体をそのまま送る。終日の予定は形が違うのでこちらを使う
async function postEvent(calendarId, body) {
...
// 終日の予定。Google は date で受け取り、終了日は「含まない」ので1日足す。
// dateTime で 00:00〜23:59 を入れると、予定表で時間帯の帯になってしまい、
// 有給のような「その日は居ない」という情報として読めなくなる。
function allDayBody({ summary, description, startsOn, endsOn, confirmed }) {
  ...
  return {
    ...
    transparency: "transparent",   // 相手の空き時間を埋めない
  };
}

/**
 * 有給などの終日の予定を、共有カレンダー側であるべき状態に合わせる。
 * syncBooking と同じく、連携が未設定でも通信に失敗しても例外は投げない。
 */
export async function syncAllDay({ record, summary, description, startsOn, endsOn, action }) {
  ...
  try {
    if (action === "delete") {
      if (record?.gcal_event_id) {
        await deleteEvent(record.gcal_calendar_id || calendarId, record.gcal_event_id);
      }
      return { calendarId, eventId: null, link: null, error: null };
    }

    const body = allDayBody({ summary, description, startsOn, endsOn, confirmed: true });
    if (record?.gcal_event_id) {
      const r = await patchEvent(record.gcal_calendar_id || calendarId, record.gcal_event_id, body);
      return { calendarId: record.gcal_calendar_id || calendarId, eventId: r.id, link: r.htmlLink, error: null };
    }
    const r = await postEvent(calendarId, body);
    return { calendarId, eventId: r.id, link: r.htmlLink, error: null };
  } catch (e) {
    console.error("[gcal] all-day sync failed:", e?.message || e);
