...
import { seed, tell } from "./hr-run.js";
...
export async function onboardOne(sb, ctx, user, m, { code, importRowId = null, source = "form", password = null } = {}) {
  ...
  // 6.5) 担当を決めて、その人たちに知らせる。
  //
  //      ここが無いと「入社日は入っているのに、当日まで誰も動かない」になる。
  //      作っただけでは、画面を開いた人しか気づけない。
  //      管理画面の「入退社」から登録したときと、同じ道を通す（lib/hr-run.js）
  if (entry.onboarding?.procedureId) {
    try {
      const people = new Map([[emp.id, { id: emp.id, name: emp.display_name }]]);
      {
        const { data: all } = await sb.from("gw_employees")
          .select("id, display_name, department").eq("tenant_id", ctx.tenantId).limit(500);
        for (const e of all || []) {
          people.set(e.id, { id: e.id, name: e.display_name, department: e.department });
        }
      }
      const proc = {
        id: entry.onboarding.procedureId, kind: "onboarding",
        target_on: m.join_date || null, notified_for: null,
      };
      await seed(sb, ctx, proc, emp, people);
      const told = await tell(sb, ctx, proc, people.get(emp.id), people, { force: true });
      entry.onboarding.told = told.people;
    } catch (e) {
      // 知らせられなくても、登録は成立している。
      // 管理画面の「入退社」を開けば、そこから配り直せる
      console.error("[onboard] 担当者に知らせられませんでした:", e?.message || e);
    }
  }
