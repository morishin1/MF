...
export async function gatherFacts(sb, tenantId, proc) {
  ...
  const [orders, signs, profile, agreed, docs, items, oriItems, oriChecks] = await Promise.all([
    ...
    soft(sb.from("gw_procedure_items").select("id, item_key, owner, required, status, title")
      .eq("procedure_id", proc.id)),
    // オリエンテーション（071）。表が無ければ null → 止めない
    soft(sb.from("gw_orientation_items").select("id, required")
      .eq("tenant_id", tenantId).eq("active", true)),
    soft(sb.from("gw_orientation_checks").select("item_id").eq("employee_id", empId)),
  ]);
  ...
  return {
    ...
    consentsOk,
    items: items || [],
    orientationOk: orientationOkOf(oriItems, oriChecks),
  };
}

/**
 * 必須の教材がすべて確認済みか。
 * 表が読めない（null）・項目が無い → 事実が無いので undefined（段階の判定は止めない）
 */
function orientationOkOf(items, checks) {
  if (!Array.isArray(items) || !items.length) return undefined;
  const done = new Set((checks || []).map((c) => c.item_id));
  return items.filter((i) => i.required !== false).every((i) => done.has(i.id));
}
...
export async function gatherFactsBulk(sb, tenantId, procs, itemsByProc) {
  ...
  const [orders, signs, profiles, agreed, docs, oriItems, oriChecks] = await Promise.all([
    ...
    soft(sb.from("gw_orientation_items").select("id, required")
      .eq("tenant_id", tenantId).eq("active", true)),
    soft(sb.from("gw_orientation_checks").select("employee_id, item_id").in("employee_id", empIds)),
  ]);
  const checksBy = new Map();
  for (const c of oriChecks || []) {
    if (!checksBy.has(c.employee_id)) checksBy.set(c.employee_id, []);
    checksBy.get(c.employee_id).push(c);
  }
  ...
  for (const p of list) {
    ...
    out.set(p.id, {
      ...
      orientationOk: orientationOkOf(oriItems, checksBy.get(p.employee_id) || []),
    });
  }
  return out;
}
