...
export function planFromNippo({ nippo, evaluation, dueDate }) {
  ...
  if (evaluation) {
    // AIが出したものは proposed。本人が採用してはじめて「やること」になる。
    // そのまま並べると、提案を片づけること自体が仕事になる（049 の頭に理由）
    if (evaluation.tomorrow_advice) {
      add(evaluation.tomorrow_advice, {
        source: "ai",
        // 本人が最優先を書いていなければ、AIの提案を一番上に置く。
        // ただし採用されるまでは画面に出ない
    ...
    for (const p of (evaluation.improvement_points || []).slice(0, 2)) {
      add(p, { source: "ai", priority: 5, status: "proposed", from_eval_id: evaluation.id });
    }
  }
...
export async function savePlan(sb, rows, nippoId) {
  ...
  const top = fresh.find((r) => r.priority === 1 && r.status !== "proposed");
  if (top) {
