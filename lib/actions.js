// 「次にやること」（ダッシュボードで自分やマネージャが手で足すぶん）と KPI。
//
// ■ 日報からは、もうここへ何も作らない
//   以前は日報の「明日の最優先」・AIの提案がここ（gw_action_items）に
//   新しい行を作っていた。いまは gw_tasks / gw_focus_days が
//   「今日の3つ・明日の3つ」のSingle Source of Truthで、日報はそこに
//   確定したものを振り返るだけ（api/nippo/index.js・lib/focus.js）。
//   同じ仕事が2つの表に生まれるのを避けるため、日報からの自動生成
//   （旧 planFromNippo/savePlan）はやめた。既存の行は消さず、
//   ダッシュボードで自分やマネージャが手で足す・閉じる用途にだけ残す。
//
// ■ 一番上に出すのはひとつだけ
...
export const shapeItem = (a) => ({
  ...
  estimateMin: a.estimate_min ?? null,
 ...
 *
 * ■ 何のための仕組みか
 *   やることが増えると、「どれから手をつけるか」を決めること自体に
 *   時間がかかる。とくに経験が浅いほど、その判断で朝が終わる。
 *   期日から逆算した順番を先に出しておき、
 *   違うと思ったら本人が入れ替える、という形にする。
 *
 * ■ 並べる順番
 *   0. ピン留め      … 本人が「今日はこれ」と決めたもの。何より上
 *   1. 期限切れ      … 昨日までが期限で、まだ終わっていない
 *   2. 今日が期限
 *   3. 期限は先だが、見積り時間から逆算すると今日から始めないと間に合わない
 *   4. 前日からの繰り越し（期限なし・昨日以前に作られた）
 *   5. のこり（期限が近い順）
 *
 * ■ 「なぜこれが上なのか」を必ず添える
 *   理由の見えない並びは、結局あとで自分で並べ直すことになる。
 *   若い人ほど、順番そのものより「なぜそうなるか」が要る。
 *
 * ■ 自動では決めきらない
 *   ここが返すのは並び順と理由だけ。
 *   本人がピン留めすれば、その判断が上に来る。
 *
 * @param {Array}  items 開いている gw_action_items（shapeItem 済み）
 * @param {string} today YYYY-MM-DD（日本時間）
 * @returns {Array} 並べ替えた items（reason / reasonLevel を足したもの）
 */
export function rankToday(items, today) {
  const day = (d) => String(d || "");
  const diffDays = (a, b) =>
    Math.round((Date.parse(`${a}T00:00:00Z`) - Date.parse(`${b}T00:00:00Z`)) / 86400000);

  // 見積りが 3時間 を超えるものは、期日の前日ではもう遅いことが多い。
  // 「何日前から始めるか」を、見積りの長さから決める
  const leadDays = (min) => {
    if (!min) return 0;
    if (min >= 8 * 60) return 3;      // 1日仕事は3日前から
    if (min >= 3 * 60) return 2;      // 半日仕事は2日前から
    if (min >= 60) return 1;
    return 0;
  };
  ...
  const scored = (items || []).map((a) => {
    ...
    const left = due ? diffDays(due, today) : null;
    const lead = leadDays(a.estimateMin);

    if (a.pinnedAt) {
      return { ...a, rank: 0, reason: "自分で今日に決めたもの", reasonLevel: "pin" };
    }
    if (left !== null && left < 0) {
      return { ...a, rank: 1, reason: `期限を ${-left} 日過ぎています`, reasonLevel: "over" };
    }
    if (left === 0) {
      return { ...a, rank: 2, reason: "今日が期限です", reasonLevel: "today" };
    }
    if (left !== null && lead && left <= lead) {
      return {
        ...a, rank: 3, reasonLevel: "lead",
        reason: `期限は${left === 1 ? "明日" : `${left}日後`}。`
          + `${Math.round(a.estimateMin / 60 * 10) / 10}時間かかる見込みなので、今日から`,
      };
    }
    // 期限が無いまま持ち越しているもの。放っておくと永久に下にいる
    if (!due && a.createdAt && a.createdAt.slice(0, 10) < today) {
      return { ...a, rank: 4, reason: "前の日から残っています", reasonLevel: "carry" };
    }
    return {
      ...a, rank: 5, reasonLevel: "later",
      reason: left === null ? "期限なし"
        : left === 1 ? "期限は明日" : `期限まであと ${left} 日`,
    };
  });

  return scored.sort((a, b) =>
    a.rank - b.rank
    // ピン留め同士は、あとから留めたものを上に（いま決めたことが最新の判断）
    || (a.rank === 0 ? String(b.pinnedAt).localeCompare(String(a.pinnedAt)) : 0)
    || day(a.dueDate || "9999-12-31").localeCompare(day(b.dueDate || "9999-12-31"))
