...
export const BROWSER_EVENT_KINDS = [
  ...
  // 端末の一生（064）。管理者が押したこと・そのPCで実際に起きたこと
  "lost", "wipe_requested", "wipe_canceled", "wiped",
  // ブラウザ拡張（067）。外したのが管理者なのか、届かなくなっただけなのか
...
export const AGENT_EVENT_KINDS = [
  ...
  // エージェント自身が、自分の重さを見て送ってくる。
  // 端末管理のソフトがPCを重くしていたら、真っ先に知りたい
...
export const EVENT_LABEL = {
  ...
  ext_unlinked: "管理者がブラウザ拡張の登録を外しました",
  ext_reinvited: "管理者が登録を外し、本人に登録のやり直しを依頼しました",
  ext_missing: "ブラウザ拡張から通信が届かなくなりました",

  lost: "紛失として止めました（資格情報を失効）",
  wipe_requested: "端末の削除を指示しました",
  wipe_canceled: "端末の削除を取り消しました",
  wiped: "このパソコンから EIGHT Agent が消えました",
  ...
  agent_load_high: "Agent負荷異常", agent_load_ok: "Agent負荷が戻りました",
};
...
export const RULE_LABEL = {
  ...
  agent_load: "Agent負荷異常（このPCを重くしています）",
};
...
export function dayVerdict({ usage = null, distractSec = 0, silent = false, workedMin = 0,
                             noDevice = false, policy = {} } = {}) {
  // 端末が1台も無い人と、あるのに届かない人を、同じ文で片づけない。
  // 前者は「まだ始めていない」、後者は「止まっている」。次にやることが違う
  if (noDevice) {
    return { key: "check", mark: "△", label: "未設定",
...
export function alertsFrom({ events = [], usage = null, policy = {} }) {
  ...
  for (const e of events) {
    ...
    //
    // これは本人の働きかたの話ではなく、**こちらの落ち度**。
    // 入れさせてもらっている以上、いちばん早く直すべきもの。
    // だから本人に確かめる類のものとは分けて、critical で出す
    if (e.kind === "agent_load_high") {
      out.push({
        severity: e.detail?.label === "critical" ? "critical" : "warn",
        rule: "agent_load",
        title: "Agent負荷異常（このPCを重くしています）",
        detail: e.detail, occurredAt: e.at, seq: e.seq,
        dedupeKey: `load:${e.workDate}`,
      });
    }
  }
...
export function deviceState(device, { critical = 0, warn = 0, staleDays = 60, now = Date.now() } = {}) {
  ...
  // 削除の指示が出ている。消えたかどうかは、そのPCから報せが来て分かる。
  // 使用終了より先に見る（削除は使用終了にしたうえで行うため）
  if (device.wipe_requested_at && !device.wipe_done_at) {
    return {
      key: "wiping", label: "削除待ち",
      note: "登録は解除しました。このパソコンが次にネットにつながったときに、"
          + "EIGHT Agent が自分を消します。消え終わると台帳から外れます",
    };
  }
  if (device.wipe_done_at) return { key: "wiped", label: "削除済み" };
  if (device.lost_at) {
    return {
      key: "lost", label: "紛失",
      note: "資格情報を失効させました。このパソコンからは何も受け取りません。"
          + "見つかったら「利用を再開する」で戻せます",
    };
  }
