...
/**
 * まだ手続きの途中にある状態。「sent」は署名依頼を出しただけで、本人の署名は
 * まだ済んでいない（＝この依頼はまだ生きている）ので含める。
 * 「signed」（締結ずみ）「cancelled」（取り消し）は終わった依頼なので含めない
 * ＝契約更新・再契約・条件変更後の再作成では、同じ社員・同じ書類種別で
 * 新しい依頼をまた作れる
 */
export const ACTIVE_ORDER_STATUS = ["requested", "uploaded", "sent"];
...
//
// ■ 何のためか
//   候補者が実際に承諾した合格通知（gw_hr_offers）と、契約書作成依頼を出す
//   時点の労働条件（gw_contracts・gw_employees）が食い違ったまま、
//   社労士へ正式に依頼させない。比較の基準は必ず「本人が承諾したoffer」。
//   gw_hr_applicants の最新値ではない（本人はそれを見ていない）

const wageLabel = (type, amount) => (amount == null ? null
  : `${type ? `${type} ` : ""}${Number(amount).toLocaleString("ja-JP")}円`);

/**
 * gw_contracts / gw_employees の現在値から、依頼フォームの自由記入欄を組み立てる。
 * 会社が既に持っている値だけを使う（本人・社労士に二度書かせない）
 */
export function conditionsFromContract(employee, contract) {
  const out = {};
  if (employee?.employment_type) out["雇用区分"] = employee.employment_type;
  if (contract) {
    out["契約期間"] = contract.fixed_term
      ? `期間の定めあり（${contract.period_from || "未定"}〜${contract.period_to || "未定"}）`
      : "期間の定めなし";
    if (contract.probation_months) out["試用期間"] = `${contract.probation_months}か月`;
    if (contract.weekly_hours) out["就業時間"] = `週${contract.weekly_hours}時間`;
    if (contract.work_place) out["就業場所"] = contract.work_place;
    if (contract.job_content) out["業務内容"] = contract.job_content;
    const wage = wageLabel(contract.wage_type, contract.wage_amount);
    if (wage) out["賃金"] = wage;
  }
  return out;
}

/** 比較する7項目。ラベル・offer側の値・現在値の出し方をここ1か所にまとめる */
const RECONCILE_FIELDS = [
  { key: "employmentType", label: "雇用形態",
    offer: (o) => o.employment_type || null, current: (e) => e.employment_type || null },
  { key: "jobTitle", label: "職種",
    offer: (o) => o.job_title || null, current: (e) => e?.position || null },
  { key: "wage", label: "給与",
    offer: (o) => wageLabel(o.wage_type, o.wage_amount),
    current: (_e, c) => wageLabel(c?.wage_type, c?.wage_amount) },
  { key: "joinDate", label: "入社日",
    offer: (o) => o.join_date || null, current: (e) => e.joined_on || null },
  { key: "weeklyHours", label: "勤務時間",
    offer: (o) => (o.weekly_hours != null ? `週${o.weekly_hours}時間` : null),
    current: (_e, c) => (c?.weekly_hours != null ? `週${c.weekly_hours}時間` : null) },
  { key: "probationMonths", label: "試用期間",
    offer: (o) => (o.probation_months != null ? `${o.probation_months}か月` : null),
    current: (_e, c) => (c?.probation_months != null ? `${c.probation_months}か月` : null) },
  { key: "contractPeriod", label: "契約期間",
    offer: (o) => (o.contract_type === "有期"
      ? `有期（${o.contract_end_date || "未定"}まで）` : "無期"),
    current: (_e, c) => (c ? (c.fixed_term ? `有期（${c.period_to || "未定"}まで）` : "無期") : null) },
];

/**
 * 承諾済みoffer（gw_hr_offers、accepted_atあり）と、現在の労働条件
 * （gw_employees・gw_contracts）を突き合わせる。
 * @returns {Array<{key,label,offerValue,currentValue}>} 食い違っている項目だけ
 */
export function reconcileOfferConditions(offer, employee, contract) {
  if (!offer) return [];
  const out = [];
  for (const f of RECONCILE_FIELDS) {
    const offerValue = f.offer(offer);
    const currentValue = f.current(employee, contract);
    if (offerValue == null || currentValue == null) continue; // 片方が無ければ比べない
    if (offerValue !== currentValue) out.push({ key: f.key, label: f.label, offerValue, currentValue });
  }
  return out;
}

/** 労働条件通知書の題名。社労士が承認・発行するときの書面 */
export const NOTICE_TITLE = "労働条件通知書 兼 雇用契約書";

/**
 * 依頼の条件（ORDER_FIELDS）から、労働条件通知書の本文を組む。
 *
 * ■ 雛形（STARTERS.employment）と違って、差し込みは要らない
 *   条件は管理者が欄ごとに書き、社労士が直したもの。それをそのまま条文にする。
 *   空の欄は【未記入：賃金】と残す。空欄のまま発行させないのは呼ぶ側（missingConditions）
 *
 * @param {object} conditions  normalizeConditions 済み
 * @param {{ companyName?:string, employeeName?:string, joinedOn?:string }} p
 * @returns {string}
 */
export function noticeBody(conditions, p = {}) {
  const c = conditions || {};
  const v = (k) => String(c[k] ?? "").trim() || `【未記入：${k}】`;
  const company = p.companyName || "【未記入：会社名】";
  const name = p.employeeName || "【未記入：氏名】";
  const lines = [
    `${company}（以下「会社」という。）は、${name}（以下「本人」という。）に対し、`,
    "労働基準法第15条に基づき以下のとおり労働条件を通知し、本人はこれを確認して雇用契約を締結する。",
    "",
  ];
  if (p.joinedOn) lines.push(`雇入れ日　${jp(p.joinedOn) || p.joinedOn}`, "");
  ORDER_FIELDS.forEach((f, n) => {
    lines.push(`第${n + 1}条（${f.key}）`, v(f.key), "");
  });
  lines.push(
    "（その他）",
    "本通知書に定めのない事項は、就業規則および関係法令の定めるところによる。",
    "就業規則は社内システム（mf.8grp.co.jp）でいつでも閲覧できる。",
  );
  return lines.join("\n");
}
