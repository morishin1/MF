...
// v2 = 労働条件通知書として読む項目（育成期間・週所定労働時間・業務範囲）を足した
export const PROMPT_VERSION = "contract_v2";

export const DOCUMENT_TYPES = ["労働条件通知書", "雇用契約書", "その他"];
...
const SYSTEM = [
  ...
  "",
  "【育成に関わる項目】",
  "この書類は、入社後3か月の育成計画を作る材料にもなります。",
  "・training_months は、試用期間とは別の「育成期間」。",
  "  「原則3か月」のような書き方があればその月数。試用期間と混同しない",
  "・weekly_hours は週の所定労働時間を数字だけで。",
  "  「週29時間」なら 29。「1日7時間・週4日」のように書かれていれば計算してよい",
  "・work_scope は、雇入れ直後に想定される業務を箇条書きで分けた配列。",
  "  「事業推進・バックオフィス、営業支援、EC関連」のように並んでいれば分割する",
  "・scope_change は「業務の変更の範囲」として書かれた文言をそのまま",
  "・training_programs は指定されている研修の名前の配列",
  "・training_review_note は、育成期間の終了時に何を確認するかとして",
  "  書かれている文言をそのまま",
].join("\n");
...
const SCHEMA = {
  ...
  properties: {
    ...
    document_type: { type: "string", description: `${DOCUMENT_TYPES.join(" / ")} のいずれか` },
    training_months: { type: ["integer", "null"], description: "育成期間（月）。試用期間とは別。無ければ null" },
    weekly_hours: { type: ["number", "null"], description: "週の所定労働時間（数字だけ）" },
    remote_ok: { type: ["boolean", "null"], description: "リモート勤務の可否。記載が無ければ null" },
    work_scope: { type: "array", items: { type: "string" }, description: "雇入れ直後の業務の候補" },
    scope_change: { type: "string", description: "業務の変更の範囲として書かれた文言。そのまま" },
    training_programs: { type: "array", items: { type: "string" }, description: "指定研修の名前" },
    training_review_note: { type: "string", description: "育成期間終了時に確認することとして書かれた文言" },
...
export function normalize(v) {
  ...
  const training = Number(v?.training_months);
  const hours = Number(v?.weekly_hours);
  const amount = Number(v?.wage_amount);
  const list = (a, n, max) => Array.isArray(a)
    ? a.map((x) => String(x).trim().slice(0, max)).filter(Boolean).slice(0, n)
    : [];
  return {
    ...
    document_type: DOCUMENT_TYPES.includes(v?.document_type) ? v.document_type : null,
    // 育成期間は試用期間と別枠。24か月を超える値は読み違いとみて捨てる
    training_months: Number.isInteger(training) && training > 0 && training <= 24 ? training : null,
    weekly_hours: Number.isFinite(hours) && hours > 0 && hours <= 80 ? hours : null,
    remote_ok: typeof v?.remote_ok === "boolean" ? v.remote_ok : null,
    work_scope: list(v?.work_scope, 12, 60),
    scope_change: text(v?.scope_change, 1000),
    training_programs: list(v?.training_programs, 8, 60),
    training_review_note: text(v?.training_review_note, 1000),
