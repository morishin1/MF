...
/** その日の翌日。日本時間の1日の区切りを作るのに使う（土日も飛ばさない） */
...
// 朝に描いた状態になれたか。
//
// 036 で一度日報から外したが、047 で戻した。
// 朝にゴールを描かせておいて夜に照らし合わせないと、朝の1分が
// 「書いて終わり」になる。照らし合わせるところまでが1日の輪。
export const SUCCESS_MET = [
  { key: "o", label: "なれた",     mark: "○" },
  { key: "d", label: "途中まで",   mark: "△" },
  { key: "x", label: "ならなかった", mark: "×" },
];
// 改善したこと。036 で外し、047 で戻した（SUCCESS_MET と同じ理由）。
// 選ぶだけで済ませ、書きたい人だけ一言を添える
...
/** 今日の「デキタ」。これ以上増やさない（欄が多いと、埋めるために薄いものを足す） */
 ...
 * 夜の「今日の成果」。
 *
 * 朝に決めるのは3件まで（決めすぎると最優先が薄まる）。
 * 夜は違う。実際にやったことが並ぶところで、そこには
 * 「やること」で できた にしたものも入ってくる。
 * ここで3件に切ると、4件目から先が黙って消える。
 * 決める数の上限と、記録できる数の上限は別のもの。
 */
const WORK_MAX = 10;
 ...
 * その行が「できた」か。
 *
 * 一言（result）を書いていなくても、できたものはできた。
 * 「やること」で済ませたものは一言が無いまま入ってくるので、
 * 文字が入っているかどうかで数えると、やった仕事が数から抜ける。
 * 古い日報には done が無いので、そのときは result を見る
 */
export const isDone = (w) => Boolean(w?.done || w?.result);

function workItems(v) {
  if (!Array.isArray(v)) return null;
  const out = [];
  for (const r of v.slice(0, WORK_MAX)) {
    if (!r || typeof r !== "object") continue;
    const task = text(r.task, 500);
    if (!task) continue;
    const result = text(r.result, 500) ?? "";
    const undone = text(r.undone_reason, 500) ?? "";
    out.push({
      task,
      target: num(r.target),
      unit: text(r.unit, 10) ?? "",
      ...
      result,
      undone_reason: undone,
      // できた かどうかを、書いた文字とは別に持つ。
      // 「やること」で済ませたものは、一言が無いまま できた になる。
      // ここで「完了」のような文字で埋めると、日報の一覧が
      // 「完了 / 完了 / 完了」になって、何をしたのか読めなくなる
      done: Boolean(r.done === true || r.done === "1" || result) && !undone,
      actual: num(r.actual),
    });
  }
  return out.length ? out : null;
...
export function normalizeNippo(body) {
  ...
  return {
    ...
    // 朝に描いた状態になれたか。○ / 途中まで / ×。
    // 点数にはしない。×が続くこと自体が相談の材料になる（db/034 の冒頭）
    success_met: SUCCESS_MET.some((s) => s.key === body?.successMet) ? body.successMet : null,

    // 今日の「デキタ」。成果ではなく、できるようになったこと
    wins: wins(body?.wins),

    // 相談事項（任意）。本人と管理者だけが見る。みんなの日報には出さない
    consult_note: text(body?.consultNote, 1000),

    // 改善したこと。選択（分類）＋ 書きたい人だけ一言
    improve_tags: tags(body?.improveTags),
    improved_note: text(body?.improvedNote, 500),
...
/** 今日のデキタ。空行は落とし、3件までにそろえる */
function wins(list) {
  if (!Array.isArray(list)) return [];
  return list
    .map((w) => String(w ?? "").trim().slice(0, 120))
    .filter(Boolean)
    .slice(0, MAX_WINS);
}

/** 改善の分類。定義にあるものだけ通す（画面から知らない値が来ても入れない） */
function tags(list) {
  if (!Array.isArray(list)) return [];
  const known = new Set(IMPROVE_TAGS.map((t) => t.key));
  return [...new Set(list.filter((k) => known.has(k)))];
}

/** 出したかどうかの最低ライン。全部空の日報を「提出済み」にしない */
export function hasContent(n) {
  const done = (n.work_items || []).some((w) => isDone(w) || w.undone_reason);
...
export function evaluateDaily(n) {
  ...
  const undone = items.filter((i) => !isDone(i) && i.undone_reason);
  ...
  // ⑦ 改善：改善したことを書いているか。
  //    書いていなければ、できなかったことに理由を付けて明日につないでいるか
  const improved = Boolean(n.improved_note || (n.improve_tags || []).length);
