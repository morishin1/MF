...
import { IMPROVE_TAGS, isDone } from "./nippo.js";
...
// v5 = 朝のゴールの達成・今日のデキタ・改善したこと を読ませる（047）
export const PROMPT_VERSION = "daily_eval_v5";
...
function userPrompt({ today, metrics, recent, blockers = [] }) {
  ...
  const metLabel = (k) =>
    ({ o: "なれた", d: "途中まで", x: "ならなかった" }[k] || "（記載なし）");

  const improvedLabel = (t) => {
    const names = (t.improve_tags || [])
      .map((k) => IMPROVE_TAGS.find((x) => x.key === k)?.label).filter(Boolean);
    const parts = [names.join("・"), t.improved_note].filter(Boolean);
  ...
  return [
    ...
    today.morning_at
      ? [
          ...
          `今日が最高に終わった状態：${today.goal_image || today.success_image || "（記載なし）"}`,
    ...
    "",
    `③ 朝に描いた状態になれたか：${metLabel(today.success_met)}`,
    "",
    // デキタは成果とは別。成果が出ない日でも、できるようになったことはある。
    // AIにここを読ませるのは、良かった点を「終わった仕事」だけから拾わせないため
    `④ 今日のデキタ：${(today.wins || []).length
      ? "\n" + (today.wins || []).map((w) => `・${w}`).join("\n")
      : "（記載なし）"}`,
    "",
    `⑤ 相談事項：${today.consult_note || "（記載なし）"}`,
    "",
    `⑥ 改善したこと：${improvedLabel(today)}`,
    "",
